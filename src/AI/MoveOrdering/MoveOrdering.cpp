...
float MoveOrderer::EvaluateMove(const Move &move, const GameState &gameState) {
    float moveEval = 0.0f;
...
std::vector<Move> MoveOrderer::GetOrderedPossibleMoves(const GameState &gameState, const PlayerColor maximizingPlayerColor) {
...
std::vector<Move> MoveOrderer::GetOrderedPossibleMoves(const GameState &gameState, const PlayerColor maximizingPlayerColor, const KillerHeuristic *killerHeuristic, const TranspositionTable *transpositionTable) {
    ...
    if(transpositionTable->HasTranspositionEntry(gameState)) {
        ...
        if(gameState.IsMoveValid(transpositionEntry.hashMove)) {
            //std::cout << "LOL" << "\n";
            movesSorted.push_back(transpositionEntry.hashMove);
        } 
    }
...
std::vector<GameState> MoveOrderer::GetOrderedChildGameStates(const GameState &gameState, const PlayerColor maximizingPlayerColor) {
    std::vector<Move> movesToSearch = gameState.GetPossibleMoves();
...
std::vector<GameState> MoveOrderer::GetOrderedChildGameStates(const GameState &gameState, const PlayerColor maximizingPlayerColor, const KillerHeuristic *killerHeuristic, const TranspositionTable *transpositionTable) {
    std::vector<Move> movesToSearch = gameState.GetPossibleMoves();
