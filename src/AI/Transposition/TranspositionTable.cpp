...
const TranspositionEntry &TranspositionTable::GetTranspositionEntry(const Piranhas::GameState &gameState) const {
...
bool TranspositionTable::HasTranspositionEntry(const Piranhas::GameState &gameState) const {
    auto iterator = table.find(ComputeKey(gameState));
...
void TranspositionTable::AddTranspositionEntry(unsigned long long zobristKey, const TranspositionEntry &transpositionEntry) {
    table.insert(std::make_pair(zobristKey, transpositionEntry));
}
